...
def test_find_op_binary_not_found(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("shutil.which", return_value=None):
        with pytest.raises(FileNotFoundError, match="op binary not found"):
            _find_op_binary()
...
def test_run_op_build_returns_build_result(workspace: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_run_op_build_no_build_result_json(workspace: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OP_BINARY", "/usr/bin/op")
...
def test_run_op_build_no_push_flag_by_default(workspace: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OP_BINARY", "/usr/bin/op")
    with _mock_subprocess(workspace) as mock_run:
...
def test_run_op_build_propagates_subprocess_error(
    ...
    monkeypatch.setenv("OP_BINARY", "/usr/bin/op")
    import subprocess
    with patch("subprocess.run", side_effect=subprocess.CalledProcessError(1, "op")):
